...
import { Edit, FileText, Eye, Copy, X } from 'lucide-react';
import { parseLatexContent as parseLatex, getLatexExample } from '../utils/latexParser';

const QuestionEdit: React.FC = () => {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const { questions, updateQuestion } = useAppStore();
  const [inputMode, setInputMode] = useState<'manual' | 'latex'>('manual');
    ...
    if (id) {
      ...
      if (question) {
        setFormData({
          ...
          custom_tags: question.custom_tags || [],
        });
      }
      setLoading(false);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await questionApi.update(id, formData);
